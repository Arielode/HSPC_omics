#### args and basic functions



level <- c("HSC/MPP", "Early myeloid progenitor", "Early lymphoid progenitor", 
	                       "MLP", "Early BNK", "Late BNK", 
			                           "MEP", "Erythroid progenitor", 
			                            "Early GMP", "Intermediate GMP", "Late GMP", "MDP", "CDP", "EBM", "Unknown")
rmoutliers <- function(dat, level){
	toplot<- vector("list", length(level))
	for(i in 1:length(level)){
		x1 <- dat[which(dat$cluster==level[i]), ] ## dat("rna"*"cluster")
		x1 <- x1[complete.cases(x1$value), ]
		x1 <- x1[!x1$value %in% boxplot.stats(x1$value)$out, ]
		toplot[[i]] <- x1
	}
	toplot<- dplyr::bind_rows(toplot)
}


cPlotOpts <- theme(axis.text = element_text(size = 15, colour = "black"), 
		   axis.title = element_text(size = 18, colour = "black"), 
		   legend.text = element_text(size = 12))

# color
col <- c("HSC/MPP"="#C20000",
	          "Early myeloid progenitor"="#736FCA",
		           "Early lymphoid progenitor"="#6F86CA",
		           "MLP"="#81BDD3",
			            "Early BNK"="#1AA2DE","Late BNK"="#204CFF", 
			            "MEP"="#E8B279",
				             "Erythroid progenitor"="#DB7C45",
				             "Early GMP"="#bcbddc",
					              "Intermediate GMP"="#D8BFD8", 
					              "Late GMP"="#bb8fce", "MDP"="#6D4BD1",
						               "CDP"="#D735F8", "EBM"="#BA55D3", 
						               "Unknown"="#ABAAA9")

clone_col <- c("clone1" = "#FF0000", "clone2" = "#EFAFAF", 
		                 "clone3" = "#FAE5D3", "clone4" = "#85C1E9", 
				                "clone5" = "#1C77E5", "clone6" = "#A7DBA5", 
				                "clone7" = "#5FA035", "clone8" = "#CCCCFF", 
						               "clone9" = "#7E57C2", "clone10" = "#9C640C", 
						               "clone11" = "#641E16", "NA" = "#E5E8E8", 
							                      "others" = "#E5E8E8")

# heatmap left rownames
